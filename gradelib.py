            ...
            msg.append(
                ...
                % (
                    (
                        color("red", "BAD ")
                        if lineno in bad
                        else color("green", "GOOD") if lineno in good else "    "
                    ),
                    lines[lineno],
                )
            )
    ...
    try:
        ...
        with open("time.txt") as f:
            ...
            if not re.match(r"^\d+$", d):
                raise AssertionError(
                    "time.txt does not contain a single integer (number of hours spent on the mp)"
                )
    except IOError:
...
class Runner:
    ...
    def match(self, *args, **kwargs):
        ...
        ans_idx = -1
        lines = self.qemu.output.split("\n")
        lines = filter(
            lambda x: not x.startswith("BW_ACTION") and not x.startswith("BW_DIAG"),
            lines,
        )
        is_ls = False
        for i, line in enumerate(lines):
            # print(i, lne, ans_idx)
            if ans_idx == -1:
                if "init: starting sh" in line:
                    ans_idx += 1
                # else:
                # print('|{}| == |{}|'.format(args[0], line))
                continue
            ans = args[ans_idx]

            if is_ls and (ans.startswith("$") or ans.startswith("ls: cannot open")):
                is_ls = False

            result = ans == line if not is_ls else self.match_ls_output(line, ans)
            if not result:
                msg = "[Error] Output on line {}: '{}', should be '{}'".format(
                    ans_idx, line, ans
                )
        ...
        if ans_idx != len(args):
            if ans_idx < 0:
                ans_idx = 0
            msg = "[Error] Missing output on line {}: {}".format(ans_idx, ans)
            raise AssertionError(msg)
        if not lines[i + 1].startswith(
            "$ qemu-system-riscv64: terminating on signal 15"
        ):
            msg = "[Error] Detect extra output on line {}: {}".format(
                i + 1, lines[i + 1]
            )
            raise AssertionError(msg)

    def match_ls_output(self, s_out, ans):
        # check the correctness of the output of `ls`
        # ignore inode number and file size
        s_out = s_out.split()
        ans = ans.split()
        return (
            len(s_out) == 5
    ...
    def match_substrings_ordered(self, *expected_substrings):
        """
        Checks if all expected_substrings appear in the most recent QEMU
        output (self.qemu.output), in the specified order.
        Intervening lines or other text in the QEMU output are ignored.
        Raises AssertionError if any substring is not found, or if they
        are found out of order.
        """
        if self.qemu is None or self.qemu.output is None:
            msg = "[Error] QEMU output is not available for match_substrings_ordered. Did run_qemu() execute successfully?"
            raise AssertionError(msg)

        qemu_full_output = self.qemu.output

        current_search_offset = (
            0  # Tracks where to start searching for the next substring
        )

        for i, sub_string in enumerate(expected_substrings):

            if not sub_string:
                # print "PYTHON_WARNING: Empty substring provided to match_substrings_ordered at index {}.".format(i)
                continue

            try:
                # Find the substring in the qemu_full_output starting from current_search_offset
                found_index = qemu_full_output.find(sub_string, current_search_offset)
            except Exception as e:  # pylint: disable=broad-except
                # Handle cases where qemu_full_output might not be a string as expected
                msg = "[Error] An unexpected error occurred while searching QEMU output in match_substrings_ordered: {}".format(
                    e
                )
                msg += "\nQEMU output type: {}".format(type(qemu_full_output))
                raise AssertionError(msg)

            if found_index == -1:
                # Substring not found after the previous one (or at all if it's the first)
                error_message_lines = [
                    "[Error] Expected output sequence mismatch in match_substrings_ordered."
                ]
                error_message_lines.append("Could not find: '{}'".format(sub_string))
                if i > 0:
                    error_message_lines.append(
                        "After successfully matching: '{}'".format(
                            expected_substrings[i - 1]
                        )
                    )

                qemu_lines_for_context = qemu_full_output.splitlines()
                # Try to find the line number where the search was happening
                approx_line_num_search_stopped = qemu_full_output.count(
                    "\n", 0, current_search_offset
                )

                context_start_line = max(0, approx_line_num_search_stopped - 5)
                context_end_line = min(
                    len(qemu_lines_for_context), approx_line_num_search_stopped + 5
                )

                error_message_lines.append(
                    "\nRelevant QEMU output (approx. lines {} to {} where '{}' was expected):".format(
                        context_start_line, context_end_line, sub_string
                    )
                )

                for k in range(context_start_line, context_end_line):
                    if k < len(qemu_lines_for_context):  # Boundary check
                        error_message_lines.append(
                            "{:4d}: {}".format(k, qemu_lines_for_context[k])
                        )

                if context_end_line < len(qemu_lines_for_context):
                    error_message_lines.append("...")

                raise AssertionError("\n".join(error_message_lines))

            # If found, update the search offset for the next substring
            current_search_offset = found_index + len(sub_string)

        # If the loop completes, all expected_substrings were found in the correct order.
        print(
            "PYTHON_INFO: match_substrings_ordered: All expected messages found in QEMU output in the correct order."
        )
